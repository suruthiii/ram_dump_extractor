...
input_folder = "extracted files"
output_folder = "extracted json files"

def parse_pslist(lines):
    events = []
    for line in lines:
        if not line.strip() or line.startswith("PID") or line.startswith("Volatility"):
            continue
    ...
    for file in os.listdir(input_folder):
        if file.endswith(".txt"):
            ...
            if events:
                ...
                print(f"[ ] Skipped {file} (no extractor or no valid events)")
